...
The worktree `/Users/zaidalbaker/w/beatzball/caribou-worktrees/02-auth-data-timeline` on branch `02-auth-data-timeline` (based on `main@f5c6442`) already exists and is the working directory for every subsequent task.
...
```bash
cd /Users/zaidalbaker/w/beatzball/caribou
git fetch origin
git checkout main
git pull --ff-only
```
